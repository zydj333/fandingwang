...
class tencent_model extends CI_Model {
    ...
    public function checkOpenidIsSet($openid) {
        $this->db->where(array('tencent_openid' => $openid));
     ...
     *
     * @param $openid  腾讯的openid
     *
     * @return 返回一个结果集
     *
     */
    public function getMemberByTencentOpenid($openid) {
        $this->db->where(array('tencent_openid' => $openid));
        $query = $this->db->get('ai_member');
        return $query->row();
    }
