    ...
    <body>
        <div class="place">
            ...
            <ul class="placeul">
                ...
                <li><a href="<?php echo base_url(); ?>admin_power/add"><?php echo lang('page_where_add'); ?></a></li>
            </ul>
        </div>
        <div class="formbody">
            <form name="system_add_save" action="<?php echo base_url(); ?>admin_power/add" method="post" id="system_add_save" enctype="multipart/form-data" >
                <div class="formtitle"><span><?php echo lang('add'); ?></span></div>
                <ul class="forminfo">
                    <li><label><?php echo lang('powername'); ?></label><input name="powername" type="text" class="dfinput" value=""  style="width:250px;"/></li>
                    <li>
                        <table id="powertable" class="form" cellpadding=0 cellspacing=0>
                            <?php if (!empty($system)): ?>
                            <?php foreach ($system as $key => $value): ?>
                                    <tr>
                                        <td><input class="key" type="checkbox" name="power[]" id="piwer_<?php echo $value->id; ?>" value="<?php echo $value->id; ?>" /><?php echo $value->titel; ?></td>
                                    </tr>
                            <?php if (!empty($value->second)): ?>
                            <?php foreach ($value->second as $k => $v): ?>
                                            <tr>
                                                <td style=" padding-left: 50px; " ><input class="key" type="checkbox" name="power[]" id="piwer_<?php echo $v->id; ?>" value="<?php echo $v->id; ?>" /><?php echo $v->titel; ?></td>
                                            </tr>
                            <?php if (!empty($v->third)): ?>
                            <?php foreach ($v->third as $n => $m): ?>
                                                    <tr>
                                                        <td style=" padding-left: 150px; " ><input class="key" type="checkbox" name="power[]" id="piwer_<?php echo $m->id; ?>" value="<?php echo $m->id; ?>" /><?php echo $m->titel; ?></td>
                                                    </tr>
                            <?php endforeach; ?>
                            <?php endif; ?>
                            <?php endforeach; ?>
