        ...
        <script type="text/javascript">
         ...
         //搜索
           ...
           function getPageUrl(nowpage){
               ...
               $.ajax({
                        type: "POST",
                        url: "/admin_mail/ajaxList/"+Math.random(),
                        ...
                        success: function(data){
                            ...
                            str+='<td>'+values.randnum+'</td>';
                            str+='<td>';
                                str+='<a onclick="return confirm(\'你确定要重置并重新发送此信息？\')" href="/admin_mail/reset/'+values.id+'" class="tablelink">重置</a>/';
                                str+='<a onclick="return confirm(\'你确定要删除此信息？\')" href="/admin_mail/del/'+values.id+'" class="tablelink">删除</a>';
                            str+='</td>';
                        str+='</tr>';
                                });
                            }
    ...
    <body>
        <div class="place">
            ...
            <ul class="placeul">
                ...
                <li><a href="<?php echo base_url(); ?>admin_mail"><?php echo lang('page_where_middle'); ?></a></li>
                <li><a href="<?php echo base_url(); ?>admin_mail/index"><?php echo lang('page_where_list'); ?></a></li>
            </ul>
                ...
                <?php if (!empty($list)): ?>
                    ...
                    <?php foreach ($list as $key => $values): ?>
                        <tr align="left">
                            ...
                            <td><?php if($values->status==0){echo '未发送';}else if($values->status==1){echo '已发送';}else if($values->status==2){echo '已使用';}?></td>
                            <td><?php echo $values->type;?></td>
                            <td><?php echo $values->randnum;?></td>
                           
                            <td>
                                <a onclick="return confirm('<?php echo lang('reset_conform'); ?>')" href="<?php echo base_url(); ?>admin_mail/reset/<?php echo $values->id; ?>" class="tablelink"><?php echo lang('reset'); ?></a>/
                                <a onclick="return confirm('<?php echo lang('del_conform'); ?>')" href="<?php echo base_url(); ?>admin_mail/del/<?php echo $values->id; ?>" class="tablelink"><?php echo lang('del'); ?></a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
