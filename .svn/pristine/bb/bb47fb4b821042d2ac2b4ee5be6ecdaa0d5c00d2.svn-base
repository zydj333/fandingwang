    ...
    <body>
        <div class="place">
            ...
            <ul class="placeul">
                ...
                <li><a href="<?php echo base_url(); ?>admin_newtype/edit/<?php echo $newtype->id?>"><?php echo lang('page_where_edit'); ?></a></li>
            </ul>
        </div>
        <div class="formbody">
            <form name="system_add_save" action="<?php echo base_url(); ?>admin_newtype/edit/<?php echo $newtype->id?>" method="post" id="system_add_save" enctype="multipart/form-data" >
                <div class="formtitle"><span><?php echo lang('add'); ?></span></div>
                <ul class="forminfo">
                    <li><label><?php echo lang('title'); ?></label><input name="title" type="text" class="dfinput" value="<?php echo $newtype->title;?>"  style="width:550px;"/></li>
                    <li><label><?php echo lang('salt'); ?></label><input name="salt" type="text" class="dfinput" value="<?php echo $newtype->salt;?>"  style="width:150px;"/></li>
                    <li><label><?php echo lang('pid'); ?></label>
                        <div class="vocation">
                            <select class="select1" name="pid">
                                <option value="0"><? echo lang('no_parent')?></option>
                                <?php foreach ($type as $k => $v): ?>
                                    <option value="<?php echo $v->id; ?>" <?php if($v->id==$newtype->pid){echo 'selected="selected"';}?> ><?php echo $v->title; ?></option>
                                <?php endforeach; ?>
                                </select>
                            </div>
                        </li>
