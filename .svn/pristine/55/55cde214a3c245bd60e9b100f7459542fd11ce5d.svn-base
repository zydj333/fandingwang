        ...
        <div class="left">
            <div class="nav">
                <ul>
                    ...
                    <li class="now">支持者(<?php echo $product->support_times;?>)</li>
                </ul>
            </div>
            <div class="con">  
                <?php if(!empty($support)):?>
                <div class="commentlist support">
                    <?php foreach ($support as $key=>$values):?>
                    <div class="item">
                        <?php if($values->uid==0):?>
                        <div class="head"><img src="<?php echo base_url();?>images/share_head.png"/></div>
                        <?php else:?>
                        <div class="head"><a href="<?php echo base_url();?>usercenter/userhome/<?php echo $values->uid;?>" target="_blank"><img src="<?php echo base_url().$values->avatar;?>" onerror="this.onerror='';this.src='<?php echo base_url();?>images/share_head.png'"/></a></div>
                        <?php endif;?>
                        <div class="reply"><?php echo date('Y-m-d H:i;s',$values->addtime);?></div>
                        <?php if($values->uid==0):?>
                        <div class="name"><a href="javascript:void(0);"><?php echo $values->username;?></a></div>
                        <?php else:?>
                        <div class="name"><a href="javascript:void(0);"><?php echo $values->account;?></a></div>
                        <?php endif;?>
                        <div class="con">支持 <?php echo $values->amount;?> 元</div>
                    </div>
                     <?php endforeach;?>
